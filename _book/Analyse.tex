...
\protect\hypertarget{def:unnamed-chunk-6}{}{\label{def:unnamed-chunk-6} \iffalse (Continuité) \fi{} }Soit \(f\) une fonction définie sur un intervalle \(I\) de \(\mathbb R\) à valeurs réelles.
On dit que \(f\) est une fonction injective de \(I\) dans \(\mathbb R\) si les images par \(f\) de deux éléments distincts de \(I\) sont différentes ou encore
\EndKnitrBlock{definition}
